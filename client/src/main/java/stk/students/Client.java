...
public class Client {
    ...
    private static boolean addOrRemoveAssignmentRole() throws RemoteException{
        ...
        FixedAnswerWindow optionsRole = new FixedAnswerWindow("assignrole.options", Color.BLUE);
        DynamicAnswerWindow userWindow = new DynamicAnswerWindow("assignrole.userhierachy");
        for(User user : users.values()){ printUser(user); }
        DynamicAnswerWindow roleWindow = new DynamicAnswerWindow("assignrole.userroleoverview");
        for(Role role : roles.values()) { printRole(role); }
